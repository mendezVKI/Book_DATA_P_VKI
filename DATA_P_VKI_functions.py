...
# Functions for all exercises in the book
...
# Compute ensemble autocorrelation.
...
# Compute Cyclic Corss Correlation 
def Cyclic_Cross_C(u,v):
  u_p=u-np.mean(u)
  v_p=v-np.mean(v)  
  C_UV=np.fft.ifft(np.abs(np.fft.fft(u_p)*np.fft.fft(v_p))).real 
  r_UV=(C_UV/len(u))/(np.std(u)*np.std(v))
  return r_UV[:len(u)//2]	

# Compute Linear Cross Correlation 
def Linear_Cross_C(u,v):
 # Call to the scipy function correlate:
 r_UU = signal.correlate(u-np.mean(u), v-np.mean(v),\
 mode='same',method='auto')/(len(u)-1)/(np.std(u)*np.std(v))
 return r_UU[r_UU.size//2:]	





def Plot_Field_TEXT_Cylinder(File, Name_Mesh, PLOT=False):
    """
